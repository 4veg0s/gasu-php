...
if ($id_from != '') {
    ...
    if (isset($_POST['deleteMessageSubmit'])) {
        ...
        $sql_delete_message_by_id = 'DELETE FROM `report` where `id` = ' . $message_id_to_delete;
    ...
    echo '<div class="reports-container" id="reports">';

    echo '<h2><center>Отзывы о пользователе ' . $user_to['F'] . ' ' . $user_to['I'] . ' ' . $user_to['O'] . '</center></h2>';
    
    $files = scandir($file_destination_path);
    $report_blocks = '';
    $next_date = '';
    foreach ($all_reports_ordered_by_creation_array as $report) {
        $current_user = $report['id_from'] == $user_from['id'] ? $user_from : $user_to;
        $img_file = '';
        foreach ($files as $file) {
        ...
        $report_author_avatar = profile_link($current_user['id'], '<img src="' . $img_route . '" class="" title="' . $current_user['F'] . ' ' . $current_user['I'] . ' ' . $current_user['O'] . '"/>');

        $next_report = '<div class="report">
            <div class="review-header">
                    <div class="report-author">' .
                        $report_author_avatar .
                        '<span>' . $current_user['F'] . ' ' . $current_user['I'] . ' ' . $current_user['O'] . '</span>' .
                    '</div>' .  // report-author
